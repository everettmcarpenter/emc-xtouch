...
// patchbay
Collage grains( "../audio/Brit-Voice-Iso.wav", 16 )[8] => Gain faders( 0.0 )[8] => dac;
// reverb
faders => Gain revSend( 0.0 )[8] => NRev reverb( 1.0 ) => dac;

for( int i; i < grains.size(); i++ ) grains[i].position( 1.0, grains[i].duration() * 2.0 );
            ...
            if( cc <= 17 )
            {
				...
				else if( cc == 16 )
                {
                    for( int i; i < grains.size(); i++ )
                    {
                        if( channelSwitch[i] )
                        {
                            ( grains[i].spacer() * midi2float( ccval ) ) + grains[i].spacer() => dur n_spacer;
                            // if the value is 0, then we'll need to give it a boost 
                            if( grains[i].spacer() == 0::samp && ccval > 0 ) 5::ms +=> n_spacer;
                            // if it's really low, we're probably just continuously moving the knob to zero out the value
                            else if( n_spacer <= 5::ms ) 0::samp => n_spacer;
                            grains[i].spacer( n_spacer );
                            <<< "grain spacer : ", grains[i].spacer() >>>;
                        }
                    }
                }
            }
